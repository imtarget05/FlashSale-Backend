...
| AI product content (spec §8) | `POST /api/products/{id}/content/generate` (StaffOrAdmin) | new draft per request | qwen3:4b generates short desc + SEO + keywords + social caption + FAQ JSON → `ReviewRequired` draft | 503 after bounded retry (2 attempts × 120s budget) | bounded retry, no infinite loop | 1 run per generate (`AiContentGeneration`) | **yes — publish requires `Approved` (409 otherwise); applied copy writes `Products.Description`** |
| Support triage (spec §9) | `POST /api/support/triage` (authenticated) | intent + optional order key | classification → PostgreSQL-grounded facts → draft reply; REFUND/CANCELLATION or ungrounded order-status ⇒ `requiresHumanReview=true` | 502 unparseable / 503 unavailable | bounded retry (2) | 1 run per triage (`SupportTriage`) | **yes — sensitive categories flagged for human review; model never invents order status** |
